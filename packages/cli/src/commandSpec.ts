...
const taskCreateFields = [value("description", "<text>", "Description."), value("deadline", "<YYYY-MM-DD>", "Local scheduled date."), value("time", "<HH:MM>", "Local scheduled time."), value("priority", "<p1|p2|p3>", "Priority."), value("tags", "<tag,...>", "Comma-separated tags."), value("estimated-minutes", "<minutes>", "Positive estimate."), value("goal", "<target>", "Exact Goal name or ID to link.")];
const taskEditFields = [value("description", "<text|clear>", "Description."), value("deadline", "<YYYY-MM-DD|clear>", "Local scheduled date."), value("time", "<HH:MM|clear>", "Local scheduled time."), value("priority", "<p1|p2|p3|clear>", "Priority."), value("tags", "<tag,...|clear>", "Comma-separated tags."), value("estimated-minutes", "<minutes|clear>", "Positive estimate."), value("goal", "<target|clear>", "Exact Goal name or ID, or clear to unlink.")];
const filters = [value("goal", "<target>", "Exact Goal name or ID."), value("priority", "<p1,p2,p3>", "Any supplied priority."), value("tag", "<tag,...>", "Any supplied tag."), value("status", "<active|inbox|timeline|completed|cancelled>", "Task lifecycle."), value("date", "<YYYY-MM-DD>", "Exact local date."), value("before", "<YYYY-MM-DD>", "Strict upper date bound."), value("after", "<YYYY-MM-DD>", "Strict lower date bound."), flag("all", "Include active and historical Tasks."), flag("long", "Show expanded human details.")];
...
export const COMMAND_SPECS: readonly CommandSpec[] = [
  ...
  { path: ["tasks", "add"], summary: "Create a Task.", kind: "write", requiredScopes: ["tasks:write"], target: { label: "<title>", kind: "task" }, options: [...taskCreateFields, ...write], supportsDryRun: true, generatedIdempotency: true },
  { path: ["tasks", "edit"], summary: "Edit a Task.", kind: "write", requiredScopes: ["tasks:write"], target: { label: "<task>", kind: "task" }, options: [value("title", "<text>", "Replacement title."), ...taskEditFields, ...write], supportsDryRun: true, generatedIdempotency: true },
  ...
  { path: ["tasks", "link"], summary: "Link a Task to a Goal.", kind: "write", requiredScopes: ["tasks:write"], target: { label: "<task>", kind: "task" }, options: [value("goal", "<target>", "Exact Goal name or ID."), ...write], supportsDryRun: true, generatedIdempotency: true },
  { path: ["tasks", "unlink"], summary: "Unlink a Task from its Goal.", kind: "write", requiredScopes: ["tasks:write"], target: { label: "<task>", kind: "task" }, options: write, supportsDryRun: true, generatedIdempotency: true },
...
export function renderCommandHelp(spec: CommandSpec) { const target = spec.target ? ` ${spec.targetOptional ? `[${spec.target.label}]` : spec.target.label}` : ""; const usage = `pravah ${getCommandName(spec)}${target}`; return [usage, "", spec.summary, "", "Options:", ...[...spec.options, ...globalOptions].map((option) => `  --${option.name}${option.kind === "value" ? ` ${option.valueLabel}` : ""}  ${option.description}`), "", `Required scopes: ${spec.requiredScopes.join(", ") || "none"}`, `Dry-run: ${spec.supportsDryRun ? "supported" : "not supported"}`, `Idempotency: ${spec.generatedIdempotency ? "generated; --idempotency-key is optional" : "not used"}`, ...(spec.confirmationFlag ? [`Confirmation: --${spec.confirmationFlag} is required`] : [])].join("\n"); }
...
export function suggestClosestCommand(input: string, candidates = getAllCommandNames()) { const value = input.trim().toLowerCase(); if (value.length < 2) return null; const prefix = value.slice(0, Math.max(2, value.length - 1)); return candidates.find((candidate) => candidate.toLowerCase().startsWith(prefix)) ?? candidates.find((candidate) => candidate.toLowerCase().includes(value.split(" ")[0]!)) ?? null; }
let cachedVersion: string | undefined;
export function readCliPackageVersion() { if (cachedVersion) return cachedVersion; try { return cachedVersion = JSON.parse(readFileSync(new URL("../package.json", import.meta.url), "utf8")).version as string; } catch { return cachedVersion = "unknown"; } }
