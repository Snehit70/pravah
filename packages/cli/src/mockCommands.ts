...
function receipt(action: string, target: { id: string; title?: string }, args: ParsedArgs) { const metadata = getWriteMetadata(args); return { action, target, ...metadata, operation: { id: `op_mock_${action.replaceAll(".", "_")}`, undoAvailable: true, expiresAt: "2026-06-04T13:00:00.000Z" }, source: "mock" }; }
