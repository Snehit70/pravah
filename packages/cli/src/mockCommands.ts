...
function filteredTasks(args: ParsedArgs) {
  ...
  return mockTasks.filter((task) => {
    ...
    if (!status && !hasFlag(args.options, "all") && !active) return false;
...
function horizon(tasks: ReturnType<typeof filteredTasks>) { const today = getLocalDateString(); const end = new Date(`${today}T12:00:00`); end.setDate(end.getDate() + 14); const until = getLocalDateString(end); const active = tasks.filter((task) => task.status === "inbox" || task.status === "timeline"); const byDue = (a: { deadline?: string; time?: string; priority?: "p1" | "p2" | "p3"; title: string }, b: { deadline?: string; time?: string; priority?: "p1" | "p2" | "p3"; title: string }) => `${a.deadline ?? "9999"}${a.time ?? "99:99"}`.localeCompare(`${b.deadline ?? "9999"}${b.time ?? "99:99"}`) || priorities[a.priority ?? "p3"] - priorities[b.priority ?? "p3"] || a.title.localeCompare(b.title); const overdue = active.filter((task) => task.deadline && task.deadline < today).sort(byDue); const todayTasks = active.filter((task) => task.deadline === today).sort(byDue); const upcoming = active.filter((task) => task.deadline && task.deadline > today && task.deadline <= until).sort(byDue); return { today, endDate: until, overdue, todayTasks, upcoming, inboxCount: active.filter((task) => task.status === "inbox").length }; }
function receipt(action: string, target: { id: string; title?: string }, args: ParsedArgs, extra?: Record<string, unknown>) { const metadata = getWriteMetadata(args); if (metadata.dryRun) return { action, target, ...extra, ...metadata, source: "dry-run" }; return { action, target, ...extra, ...metadata, operation: { operationId: `op_mock_${action.replaceAll(".", "_")}`, undoAvailable: true, undoExpiresAt: "2026-06-04T13:00:00.000Z" }, source: "mock" }; }
function mockGoalChange(args: ParsedArgs, editing: boolean): { id: string; text: string } | null | undefined {
  const raw = readOption(args.options, "goal");
  if (raw === undefined) return undefined;
  const cleaned = raw.trim();
  if (!cleaned) throw new Error("--goal requires a value");
  if (["clear", "none", "null"].includes(cleaned.toLowerCase())) {
    if (!editing) throw new Error("--goal cannot be cleared on tasks add; omit it instead");
    return null;
  }
  const goal = goalTarget(cleaned);
  return { id: goal.id, text: goal.text };
}
...
export function executeMockCommand(command: string, args: ParsedArgs): unknown {
  ...
  if (command === "tasks show") return { task: enrich(taskTarget(readTarget(args, command))), source: "mock" };
  ...
  if (command.startsWith("tasks ")) { const verb = command.slice(6); if (verb === "link") { const target = taskTarget(readTarget(args, command)); const goal = mockGoalChange(args, false); if (goal === undefined || goal === null) throw new Error("tasks link requires --goal"); return receipt("tasks.link", target, args, { goal }); } if (verb === "unlink") { if (readOption(args.options, "goal") !== undefined) throw new Error("tasks unlink takes no --goal; it removes the current link"); return receipt("tasks.unlink", taskTarget(readTarget(args, command)), args); } const target = verb === "add" ? { id: "task_mock_new", title: readTarget(args, command) } : taskTarget(readTarget(args, command)); const goal = verb === "add" || verb === "edit" ? mockGoalChange(args, verb === "edit") : undefined; if (verb === "remove" && !hasFlag(args.options, "confirm")) throw new Error("--confirm is required for tasks remove"); return receipt(`tasks.${verb}`, target, args, goal === undefined ? undefined : goal === null ? { goal: null } : { goal }); }
