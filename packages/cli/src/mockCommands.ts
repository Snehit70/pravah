...
function receipt(action: string, target: { id: string; title?: string }, args: ParsedArgs) { const metadata = getWriteMetadata(args); if (metadata.dryRun) return { action, target, ...metadata, source: "dry-run" }; return { action, target, ...metadata, operation: { operationId: `op_mock_${action.replaceAll(".", "_")}`, undoAvailable: true, undoExpiresAt: "2026-06-04T13:00:00.000Z" }, source: "mock" }; }
