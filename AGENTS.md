...
- Pravah uses `combative-zebra-261` (`https://combative-zebra-261.eu-west-1.convex.cloud`) as the canonical Convex deployment for local development, EAS builds, OTA bundles, and release workflows.
- The previous Cloud deployment remains a rollback option. Do not run two writable production deployments at once.
- If a release-control function is missing from `combative-zebra-261`, deploy the backend there before publishing a mobile release. Do not “fix” the mismatch by changing the app to another deployment.
