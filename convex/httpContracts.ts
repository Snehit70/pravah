...
export const createTaskSchema = z.object({
  ...
  if (value.time && !value.deadline) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["time"],
      message: "Time requires a deadline",
    });
  }
});
...
export const updateTaskSchema = z.object({
  ...
  time: z.string().regex(timeRegex, "Invalid time format (HH:MM)").optional().nullable(),
  ...
  if (value.time !== undefined && value.time !== null && value.deadline === null) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["time"],
      message: "Time cannot be set when clearing deadline",
    });
  }
});
