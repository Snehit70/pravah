...
3. **Newer = semver from the tag.** Strip `mobile-v`, compare `MAJOR.MINOR.PATCH` numerically against `expo-application`'s `nativeApplicationVersion`. An unparseable current version returns `malformed-metadata`; unparseable release tags are filtered out before comparison as a fail-safe.
