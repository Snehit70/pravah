...
4. **Card content: full rows, inner vertical scroll.** Each day card has a header (relative label + weekday · date + task count) and task rows led by the checkbox with a stacked body — title, one-line description, then a goal · priority meta line. *(Revised 2026-07-05 during implementation: the original single-line "slim row" wasted the fixed-height card's space and truncated titles; the on-device review chose full rows.)* Rows scroll vertically inside the fixed-height card. Tapping a row opens the Edit sheet, same as compact mode.
